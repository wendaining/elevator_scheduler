...
- [ ] 创建 `ControlPanel.vue`：
  ...
  - **调度算法切换**：`el-select` 下拉菜单：
    - 显示名称映射：`first-available` → "优先空闲算法"、`nearest-idle` → "最近优先算法"、`fcfs` → "先来先服务算法"、`scan` → "SCAN 电梯算法"
    - 切换时调用 `setScheduler`
  - **Cabin 请求**：
    ...
    - 不限制目标楼层范围
  - **日志终端**：底部一个深色区域，显示最近的事件日志（请求创建、电梯到站、状态变化等），最多保留最近 N 条
- [ ] 整体布局：`App.vue` 使用左侧 flex: 3（约 75%）+ 右侧 flex: 1（约 25%）
- [ ] **验证**：所有控件可用，调节滑块/切换算法后 state 相应变化
