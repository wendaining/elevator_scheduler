...
- [x] 给核心调度逻辑写单元测试
- [x] 测试非法楼层、边界楼层、重复请求等情况
- [x] 给 API handler 写基础测试，或者至少保留可复现的 `curl` 示例
- [x] 增加必要日志，能看出请求进入、调度分配、电梯移动、到达楼层
- [x] 确保 `go test ./...` 可以通过
- [x] 做一次小提交，例如 `test: add scheduler tests`
...
**核心思想**：电梯沿一个方向一直走到底（或到顶/底），沿途服务所有同方向的请求，到了尽头再反向。

**实现要点**：
1. 去掉 `len(e.Stops) == 0` 的限制——电梯运行时也能接单
2. 每个电梯维护一个 `ScanDirection`（已在 model 中有），空闲时默认向上
3. 分配请求时，找 **正在沿请求方向移动且请求楼层在前进方向上** 的电梯（顺路）；没有顺路的就找最近空闲的
4. 新 stop 按扫描方向 **有序插入** 电梯的停靠列表（上行时按楼层升序，下行时降序）
5. 电梯走完当前方向所有 stop 后，切换 `ScanDirection` 反向

**效果**：单部电梯可以同时服务多个请求，平均等待时间大幅下降。SCAN 是最经典的磁盘/电梯调度算法，公平性好，不会饥饿。

### LOOK

- [x] 完成

**核心思想**：和 SCAN 几乎一样，唯一的区别是——电梯不一定要走到最顶层/最底层，**只走到当前方向上最远的那个请求**，没请求了就掉头。

**实现要点**：
1. 结构与 SCAN 完全相同
2. 唯一的区别在方向切换的判定：SCAN 走到物理尽头（1 楼或顶楼）才反转；LOOK 看的是"前方还有没有请求"，没有就反转

**效果**：比 SCAN 少走冤枉路。比如电梯在 3 楼往上，最高请求在 8 楼，那它到 8 楼就掉头，不会空跑到 20 楼。LOOK 是 SCAN 的自然优化，现实中大多数电梯用的就是 LOOK 的思想。
